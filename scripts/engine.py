...
class Engine:
    ...
    def __init__(
        ...
        self.stage1 = stage1_model.eval().to(device)
        self.s2_main = stage2_main.eval().to(device)
        self.s2_supp = stage2_support.eval().to(device)
    ...
    def _stage1_infer_xy(self, cur_full: torch.Tensor, past_full_list: List[Optional[torch.Tensor]],
        ...
        if self.use_amp:
            with torch.autocast(self.device, dtype=torch.float16):
                out = self.stage1(cur300, past300, pos)
        else:
            out = self.stage1(cur300, past300, pos)
        # out = self.stage1(cur300, past300, pos)
