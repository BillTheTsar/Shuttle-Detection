...
from stage2_model_v7 import Stage2ModelBiFPN
...
def main():
    ...
    ap.add_argument("--lr-backbone", type=float, default=3e-6)
    ...
    train_loader = get_stage2_loader(
        ...
        train_mode=True, noise_std=0.02, position_perturbation=0.0
    )
    ...
    sd = torch.load(Path(PROJECT_ROOT / "models/stage2_v7_best_epoch160.pth"))
    model.load_state_dict(sd)
    print(f"Loaded weights from {Path(PROJECT_ROOT / "models/stage2_v7_best_epoch160.pth")}")
    ...
    torch.save(model.state_dict(), args.save_dir / "stage2_v7_last.pth")
    writer.close()
