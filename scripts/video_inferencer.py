...
from engine import Engine  # your engine returns (gx, gy, gv, calib_triggered)
...
def main(video_path: str, out_csv: str, device="cuda", mode="smart"):
    ...
    stage1 = Stage1ModelBiFPN(tau=1.0)
    stage2_main = Stage2ModelHeatmap(tau=1.0)   # main model
    stage2_supp = Stage2ModelBiFPN(tau=0.7)     # support model

    PROJECT_ROOT = Path(__file__).resolve().parent.parent
    stage1_ckpt = Path(PROJECT_ROOT / "models/stage1_hm_best_epoch25.pth")
    stage2_main_ckpt = Path(PROJECT_ROOT / "models/stage2_hm_best_epoch260.pth")
    stage2_supp_ckpt = Path(PROJECT_ROOT / "models/stage2_v7_best_epoch160.pth")

    stage1.load_state_dict(torch.load(stage1_ckpt, map_location=device))
    stage2_main.load_state_dict(torch.load(stage2_main_ckpt, map_location=device))
    stage2_supp.load_state_dict(torch.load(stage2_supp_ckpt, map_location=device))
    print("✅ Models loaded")
