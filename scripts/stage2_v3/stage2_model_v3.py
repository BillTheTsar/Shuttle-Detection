...
class Stage2Model(nn.Module):
    ...
    def __init__(self):
        ...
        # Normalization to match the pretrained EfficientNet B0 backbone
        ...
        self.head_xy = nn.Sequential(
            nn.Linear(512, 128),
            nn.ReLU(),
            nn.Linear(128, 2)  # raw xy (no sigmoid)
        )

        with torch.no_grad():
            self.head_xy[-1].bias.fill_(0.5)

        self.head_vis = nn.Sequential(
            nn.Linear(512, 128),
            nn.ReLU(),
            nn.Linear(128, 1)  # visibility logit
        )

        p_vis = 0.65
        with torch.no_grad():
            self.head_vis[-1].bias.fill_(torch.log(torch.tensor(p_vis/(1-p_vis))))
    ...
    def forward(self, current_crop, past_crops, positions):
        ...
        h = self.trunk(fused)  # [B,trunk_hidden]
        # heads
        xy = self.head_xy(h)  # [B,2]  (raw)
        vis_logit = self.head_vis(h).squeeze(-1)  # [B]
        return {"xy": xy, "vis_logit": vis_logit}
