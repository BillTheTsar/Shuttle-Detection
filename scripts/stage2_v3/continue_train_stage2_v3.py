...
def main():
    ...
    parser.add_argument("--lr-backbone", type=float, default=2e-4)   # discriminative LR
    ...
    def get_dataloaders():
        train_loader = get_stage2_loader(
            ...
            train_mode=True, position_perturbation=0.005
        )
    ...
    sd = torch.load(Path("storage/checkpoint/stage2_last.pth"))
    model.load_state_dict(sd)
    print(f"Loaded weights from {Path('storage/checkpoint/stage2_last.pth')}")

    # Checking that biases make sense
    with torch.no_grad():
        batch = next(iter(train_loader))
        current_img = batch["current_crop"].to(CONFIG["device"])
        past_imgs = batch["past_crops"].to(CONFIG["device"])
        positions = batch["positions"].to(CONFIG["device"])
        out = model(current_img, past_imgs, positions)
        print("Init mean xy:", out["xy"].mean(dim=0).tolist())
        print("Init vis prob mean:", torch.sigmoid(out["vis_logit"]).mean().item())
    ...
    head_decay, head_no = [], []
    for m in (model.trunk, model.attn_pool, model.traj_encoder, model.head_xy, model.head_vis):
    ...
    optimizer = optim.AdamW(
        [
            # Backbone: discriminative LR, split decay / no-decay
            ...
            # Heads (shared LR): trunk+attn+traj+xy+vis
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_vis = F.mse_loss(pred_vis, target[:, 2] * 0.80 + 0.10)
