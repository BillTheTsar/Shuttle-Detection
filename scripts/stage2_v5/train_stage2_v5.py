...
from stage2_model_v5 import Stage2ModelHeatmap
...
def freeze_backbone_bn(module, freeze_affine=False):
    for m in module.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()  # use running_mean/var; stop updating them
            if freeze_affine:  # optional: also stop learning gamma/beta
...
def main():
    ...
    ap.add_argument("--save-dir", type=Path, default="/storage/checkpoints")
    ...
    train_loader = get_stage2_loader(
        ...
        train_mode=True, position_perturbation=0
    )
    ...
    # Freeze BN statistics in the EfficientNet backbone
    freeze_backbone_bn(model.backbone.features, freeze_affine=False)
    ...
    for epoch in range(args.epochs):
        ...
        if val_total < best_val:
            best_val = val_total
            torch.save(model.state_dict(), args.save_dir / "stage2_hm_best_epoch30.pth")
